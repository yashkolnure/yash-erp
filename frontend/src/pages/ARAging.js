...
function CustomerRow({ row, buckets }) {
    const [open, setOpen] = useState(false);

    // Collect all invoices for this customer across buckets
    const customerInvoices = BUCKET_LABELS.flatMap(b =>
        (buckets[b.key] || []).filter(inv => inv.customer === row.customer).map(inv => ({ ...inv, bucket: b.label, bucketSx: b.sx }))
    );
    ...
    return (
        <>
            ...
            >
                ...
                <TableCell sx={{ fontWeight: 600 }}>{row.customer}</TableCell>
                    ...
                    <Collapse in={open} timeout="auto" unmountOnExit>
                        <Box sx={{ bgcolor: '#F8FAFC', px: 3, py: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, mb: 1, display: 'block' }}>
                                Invoices for {row.customer}
                            </Typography>
                            <Table size="small">
                                <TableHead>
                                    <TableRow>
                                        <TableCell sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Invoice #</TableCell>
                                ...
                                <TableBody>
                                    {customerInvoices.map(inv => (
                                        <TableRow key={inv._id}>
                                            <TableCell sx={{ fontSize: '0.78rem' }}>{inv.invoice_number || '—'}</TableCell>
                                            <TableCell sx={{ fontSize: '0.78rem' }}>
                                                {inv.due_date ? new Date(inv.due_date).toLocaleDateString() : '—'}
                                            </TableCell>
                                            <TableCell>
                                                <Chip label={inv.bucket} size="small" sx={{ fontSize: '0.7rem', ...inv.bucketSx }} />
                                            </TableCell>
                                            <TableCell align="right" sx={{ fontSize: '0.78rem' }}>
                                                {inv.days_overdue > 0 ? inv.days_overdue : '—'}
                                            </TableCell>
                                            <TableCell align="right" sx={{ fontSize: '0.78rem' }}>{fmtCurrency(inv.total_amount)}</TableCell>
                                            <TableCell align="right" sx={{ fontSize: '0.78rem', fontWeight: 600 }}>{fmtCurrency(inv.amount_due)}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
...
export default function ARAging() {
    ...
    const runReport = useCallback(async () => {
        ...
        setError('');
        try {
            const res = await get(`/${selectedCompanyId}/reports/ar-aging?as_of_date=${asOfDate}`);
            setData(res?.data);
        } catch (e) {
    ...
    return (
        <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
                <AssessmentRoundedIcon sx={{ color: '#34D399', fontSize: 28 }} />
                <Box sx={{ flex: 1 }}>
                    <Typography variant="h5" sx={{ fontWeight: 700 }}>AR Aging Report</Typography>
                    ...
                    <Paper>
                        {data.summary.length === 0 ? (
                            <Box sx={{ p: 5, textAlign: 'center' }}>
                                <Typography color="text.secondary">No outstanding customer invoices found for this date.</Typography>
                            </Box>
                        ) : (
                            <Table size="small">
                                <TableHead>
            ...
            {!data && !loading && !error && (
                <Paper sx={{ p: 6, textAlign: 'center', color: 'text.secondary' }}>
                    ...
                    <Typography>Select a date and click Run Report to view AR aging.</Typography>
                </Paper>
            )}
        </Box>
    );
